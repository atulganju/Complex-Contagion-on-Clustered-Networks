...
def recolor_nodes(output, num_nodes):
    return ['red' if node['is_active'] else 'black' for node in output['nodes'][:num_nodes]]
...
def parse_args(argv):
    ...
    for opt, arg in opts:
        ...
        if opt in ('-i', '--ipath'):
            ...
            width = float(arg)
    return path, num_nodes, scale, width


def main(argv):

    path, num_nodes, scale, width = parse_args(argv)
    ...
    if num_nodes is None:
        num_nodes = len(output_jsons[0]['nodes'])

    dtree = depth_tree(output_jsons[0], num_nodes)
    G, color_map = gen_graph(output_jsons[0], num_nodes)
    ...
    for i, output in enumerate(output_jsons[:-1]):
        color_map = recolor_nodes(output, num_nodes)
